...
| ID | Requirement | Notes |
|----|-------------|-------|
| FR-X1 | **Endpoints:** `POST /api/flows/:id/run` (body: optional `input` / `datasets`) and `POST /api/flows/run` (body: `flowName`, optional `input` / `datasets`). | **Implemented.** Run-by-name returns **`flowId`** on success. **409** if more than one flow shares the same name (caller must use id or rename). |
| FR-X2 | **Request body:** Caller may supply **`datasets`** (named payloads for collection `source` nodes) and/or legacy **`input`** (unbound Source / Input nodes). If both are omitted, sources use **saved** sample `data.json` (same as UI). | **Implemented.** Prefer `datasets` for multi-source flows. Legacy `input` does **not** override named sources. |
